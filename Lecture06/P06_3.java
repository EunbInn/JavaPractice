...
//20210427 김은비 영수증 메서드 분리
public class P06_3 {
	//전역변수 스테틱으로 선언
	static Date k08_date = new Date(); // 데이트 오브젝트 선언
	static String[] k08_itemName = {"aaaaa엑스트라버진올리브오일","비비고 김치만두","안주나라 불막창볶음","다크초코마들렌 500g","삼립 촉촉한 모닝 식빵",
	...
	static int[] k08_itemPrice = {3000,2000,7300,5600,3000,
	...
	static int[] k08_itemNum = {1,1,3,1,3,1,2,3,2,1,//인티저 배열에 스트링 배열과 동일한 크기로 아이템 수량 입력
					2,2,1,2,1,2,4,3,1,2,
					1,3,2,4,1,2,1,2,4,2,3,1};
	// 불린 배열에 스트링 배열과 동일한 크기로 true false 입력 -> 해당 값은 면세, 과세 항목 구분을 위함
	static boolean[] k08_TaxFree = {false,false,false,true,false,false,false,false,false,true,
			true,false,false,false,false,false,false,false,false,true,
			true,false,false,false,false,true,true,false,false,true,false,false};
	static DecimalFormat k08_df = new DecimalFormat("###,###,###,###");//가격의 세자리마다 콤마를 출력해주기 위해 숫자 포맷 설정
	static int k08_sumNoTax = 0;//비과세항목 총 가격 변수 인티저로 선언
	static int k08_sumTax = 0;//과세항목 총 가격 변수 인티저로 선언 
	static int k08_origin = 0; //origin price 인티저 타입 선언 및 초기화 
	static int k08_tax = 0;//tax 인티저 타입 선언 및 초기화
	static int k08_totalSum = 0;// 인티저 타입 선언 및 초기화
	static String k08_taxCheck = "";//스트링 변수 선언 및 초기화
	
	public static void main(String[] args) {//메인에서 시작
		int k08_itemCount = k08_itemName.length;//아이템개수 = 아이템 이름 배열 길이
		k08_TitlePrint();//지정명, 지점 주소, 교환환불 안내 등 출력
		k08_TimeStemp();//구매일시 출력
		k08_HeaderPrint();//상품명, 단가 수량등 출력
		for (int k08_i = 0; k08_i < k08_itemCount; k08_i++) {//아이템 개수만큼 반복문 실행
			k08_itemPrint(k08_i);//아이템 관련 내용 프린트
		}
		k08_TotalPrint();//총금액(면세,과세,부가세, 총계) 프린트
	}

	public static void k08_TitlePrint() {//타이틀 부분을 출력 할 스테틱 메소드 선언
	...
	public static void k08_TimeStemp() {//구매일시를 출력할 스태틱 메소드 선언
	...
	public static void k08_HeaderPrint() {//헤더를 출력 할 스테틱 메소드 선언
		System.out.printf("-----------------------------------------\n");// 구분선 출력
		System.out.printf("  상 품 명          단  가  수량   금  액\n");// 값 구분 카테고리 출력
		System.out.printf("-----------------------------------------\n");// 구분선 출력
	}

	public static void k08_itemPrint(int k08_i) {//아이템 부분을 출력할 메소드 선언(인자 integer type i)
		itemPrice(k08_i);//아이템 총가격 계산하는 메소드 호출(인자 integer type i)
		...
		if (k08_df.format(k08_itemPrice[k08_i]).length() > 8) {//만약 아이템 가격을 콤마포함 포맷으로 변형한 길이가 8보다 길면
			...
			//출력부 숫자 길이 11로 늘리고 순차적으로 상품명, 단가, 수량, 금액 출력(금액과 관련된 숫자는 미리 지정해둔 콤마 추가 포맷으로 변경하여 출력
			System.out.printf("%s%11.11s%2.3s%11.11s\n",k08_strForm(k08_taxCheck, k08_length), 
					k08_df.format(k08_itemPrice[k08_i]), k08_itemNum[k08_i] + "",
					k08_df.format(k08_itemPrice[k08_i] * k08_itemNum[k08_i]));
		}
		else {
			//출력부 숫자 길이 10로 지정하고 순차적으로 상품명, 단가, 수량, 금액 출력(금액과 관련된 숫자는 미리 지정해둔 콤마 추가 포맷으로 변경하여 출력
	...
	public static void itemPrice(int k08_i) {//아이템 가격을 계산하는 메소드 선언(인자 integer i)
	...
	public static void k08_TotalPrint() {//계산된 가격을 출력하는 메서드 선언
		CalculateTotal(); //총 가격을 계산하는 메서드 호출
		...
		System.out.printf("-----------------------------------------\n");//구분선 출력
		k08_TimeStemp2();//주차시간 출력
		System.out.printf("-----------------------------------------\n");//구분선 출력
		k08_TimeStemp3();//바코드부분 날짜출력
	}
	
	public static void CalculateTotal() {//과세금액, 부가세, 총액을 계산하는 메서드 선언
		k08_origin = (int)(k08_sumTax / 11.0 * 10);//과세항목 누적값을 1.1로 나눈 몫을 원가 변수에 저장
		k08_tax = k08_sumTax - k08_origin;//세금 = 총액 - 원가
		k08_totalSum = k08_sumNoTax + k08_sumTax;//total sum 변수에 비과세항목 가격과 과세항목 가격 누적값의 합 저장
	}

	public static void k08_TimeStemp2() {//주차시간 출력하는 메서드 선언
	...
	public static void k08_TimeStemp3() {//바코드부분 출력하는 메서드 선언
	...
	public static String k08_strForm(String k08_temp, int k08_length) { // 스트링타입과 인티저타입을 인자로 받아 스트링을 리턴하는 매서드 선언
		int k08_blank = 0;//인티저타입 변수 선언 및 초기화
